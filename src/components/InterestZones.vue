...
import { ref, onMounted, shallowRef, computed, onBeforeUnmount } from 'vue';
...
onMounted(() => {
  ...
  // Open initial popup on mount (Yosemite Valley is a nice central choice if it exists)
  if (locations.value[4] && locations.value[4].leafletInstance) {
    locations.value[4].leafletInstance.openPopup();
    activeLocationId.value = 4;
  } else if (locations.value[0] && locations.value[0].leafletInstance) {
    locations.value[0].leafletInstance.openPopup();
